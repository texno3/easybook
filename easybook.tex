...
\begin{cnltxlist}
  ...
  \end{ctexexam}
  \command{PushFntBottom}\newtagv[Changed]{2025z}%
  默认情况下当具有 \code{b} 参数的底部浮动体与脚注同时存在时，脚注将位于浮动体上部。此命令可使脚注始终位于页面底部，与 \keyis{footnotetype}{bottom} 作用相同。\hologo{LaTeX} 在 \textbf{2025/06/01} 之后的版本中为脚注提供了套接字（socket）机制，此命令为过时命令，相当于分配插头 \code{floats-space-footnotes}，且在早期版本中不再提供功能。设置底部浮动体与脚注排列方式的插座共有6种插头：
  \begin{ctexexam}
...
\begin{changelog}[title = 更新日志]
  ...
  \shortversion{v = 2025z,date = 2025/07/16,changes = 取消对底部浮动体与脚注排列方式的改动，使用 \hologo{LaTeX} 新增的套接字机制。}
\end{changelog}
\nocite{*}
